...
function Nav() {
  const [isMobileDropdownHidden, setIsMobileDropdownHidden] =
  ...
  return (
    <div className="fixed w-full z-30 flex items-center text-white">
      ...
      <div className="md:hidden flex flex-col w-full px-4 pt-4">
        <div className="w-full flex content-center justify-between gap-8">
          ...
          <div className="flex content-center justify-end bg-black rounded-b-[30px] rounded-l-[30px] p-2 px-6">
            <button
              ...
              className="bg-black"
              onClick={() => setIsMobileDropdownHidden(!isMobileDropdownHidden)}
            >
        ...
        <div
          className={`${isMobileDropdownHidden ? 'hidden' : 'flex'} flex-col place-self-end w-[50vw] text-right mt-4 gap-4 text-bold uppercase tracking-widest text-sm bg-black p-4 rounded-xl`}
        >
